...
if (isset($_POST["Update"])) {
	...
	$sql=mysql_query("UPDATE `paper presentation` SET `Std_name`='".$Std_name."',`Branch`='".$Branch."',`Class`='".$Class."',`Event`='".$Event."',`Venue`='".$Venue."',`Date`='".$Date."' WHERE `ID`=".$ID."");
	if ($sql) {
...
<?php 
 if(isset($_GET["edit"])){
 	$sql=mysql_query("SELECT * from `paper presentation` where ID=".$_GET['edit']."");
 ...
 <form method="POST">
  ...
  <table style="width: 35%; height: 60%; background-color: #f0f0f0; border-color: #800000; margin-top: 60px; border-top-style: hidden; border-radius: 15px;" align="center"  class="table table-hover" >
     <tr><td colspan="2"><a href="report1.php"><input type="Button" style="margin-left: 365px;border-style: hidden; font-weight: 600; cursor: pointer; color: black;" name="cancel" value="X"></a><h2 style="font-family: montserrat;"><b><center>Edit Information</center></b></h2></td></tr>
     ...
     <tr><td style="color: #adadad; ">Class:</td><td><input type="text" name="new_Class" value="<?php echo $result["Class"]; ?>" style="border-color: white" class="form-control" required=""></td></tr>
       ...
       <tr><td style="color: #adadad; ">Year:</td><td><input type="text" style="border-color: white" name="new_Date" value="<?php echo $result["year"]; ?>" class="form-control" required=""></td></tr>
