...
if (isset($_POST["save"])) {
  ...
  $dob=strtotime($dob);
  $dob=date('d-m-y',$dob);
$sql=mysql_query("INSERT INTO `sport event` (`Std_name`, `Branch`, `Class`, `Event`, `Venue`, `Date`) VALUES('".$_POST["Std_name"]."', '".$_POST["Branch"]."', '".$_POST["Class"]."', '".$_POST["Event"]."', '".$_POST["Venue"]."' , '".$dob."')");
if($sql){
...
<body style="background-image: url(clg5.jpg); background-repeat: no-repeat; background-attachment: fixed; background-size: cover;">
 <form method="post">
  ...
  <table style="width: 30%; height: 60%; background-color: #f0f0f0; border-color: #800000; margin-top: 60px; border-top-style: hidden; border-radius: 15px;" align="center"  class="table table-hover" >
      ...
      <tr><td colspan="2"><a href="Home_page.php" data-toggle="tooltip" data-placement="bottom" title="Back"><input type="Button" style="margin-left: 400px; border-style: hidden; font-weight: 600; cursor: pointer; color: black;" name="cancel" value="X"></a><h2 style="font-family: montserrat;"><b><center>Sport Event</center></b></h2></td></tr>
