...
if (isset($_POST["Update"])) {
	...
	$Winner=$_POST["new_Winner"];
	$Venue=$_POST["new_Venue"];
	$Date=$_POST["new_Date"];
  $Date=strtotime($Date);
  $Date=date('d-m-Y',$Date);
  $year=$_POST["new_year"];
	$sql=mysqli_query($conn,"UPDATE `Report` SET `Std_name`='".$Std_name."',`Branch`='".$Branch."',`Class`='".$Class."',`Event`='".$Event."',`Winner`='".$Winner."',`Venue`='".$Venue."',`Date`='".$Date."',`year`='".$year."'WHERE `ID`=".$ID."");
	if ($sql) {
...
<html>
<head>
	<title>Edit Report Information</title>
...
<?php 
 if(isset($_GET["edit"])){
 	$sql=mysqli_query($conn,"SELECT * from report where ID=".$_GET['edit']."");
 ...
 <form method="POST">
  ...
  <table style="width: 35%; height: 60%; background-color: #f0f0f0; border-color: #800000; margin-top: 60px; border-top-style: hidden; border-radius: 15px;" align="center"  class="table table-hover" >
     <tr><td colspan="2"><a href="report1.php" data-toggle="tooltip" data-placement="bottom" title="Back"><input type="Button" style="margin-left: 430px;border-style: hidden; font-weight: 600; cursor: pointer; color: black;" name="cancel" value="X"></a><h2 style="font-family: montserrat;"><b><center>Edit Information</center></b></h2></td></tr>
     ...
     <tr><td style="color: #adadad; ">Class:</td><td><input type="text" name="new_Class" value="<?php echo $result["Class"]; ?>" style="border-color: white" class="form-control" required=""></td></tr>
  	  <tr><td style="color: #adadad; ">Event Name:</td><td><input type="text" style="border-color: white" name="new_Event" value="<?php echo $result["Event"]; ?>" class="form-control" required=""></td></tr>
  	   <tr><td style="color: #adadad; ">Result:</td><td><select name="new_Winner" value="<?php echo $result["Winner"]; ?>" style="border-color: white" class="form-control" required="">
        <option>Select Result</option>
        <option value="Winner">Winner</option>  
        <option value="Runner Up">Runner Up</option>
        <option value="Third">Third</option>
        </select></td></tr>
       <tr><td style="color: #adadad; ">Sponsoring Authority Or Venue:</td><td><input type="text" style="border-color: white" name="new_Venue" value="<?php echo $result["Venue"]; ?>" class="form-control" required=""></td></tr>
       ...
       <tr><td style="color: #adadad; ">Academic Year</td><td><input type="text" style="border-color: white" name="new_year" value="<?php echo $result["year"]; ?>" class="form-control" required=""></td></tr>
       <tr><td colspan="2" align="center"><input type="submit" style="width: 75%;
  height: 50px; border: 1px solid; background: blue; border-radius: 25px; font-size: 18px; color: #e9f4fb; font-weight: 700; cursor: pointer; outline: none; border-color: #2691d9; transition: .5s;" value="Update" name="Update" class="btn btn-primary"></a></td></tr>
  </table>
</form>
<?php
 }
